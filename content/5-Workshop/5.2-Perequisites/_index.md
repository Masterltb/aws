...
- Create & configure an AWS account
...
4. Choose a Support Plan (Free tier is available)
5. Verify your email & set up billing information
...
2. Enter your root account email & password
3. Verify MFA (Multi-Factor Authentication) if prompted

**Note**: It's recommended to enable MFA for the root account for security.

---

## Part 2: IAM Setup - Create an Admin User
...
### Step 2: Create an IAM User for the Workshop
...
2. Find and check the following policy:
   - AdministratorAccess (allows access to all services)
...
### Check Access to AWS Services

1. Log in to the AWS Console with the IAM user
...
**Check**: You should see "Create application" (not an error message).

![Permission Verification](../assets/02-permissions-verification.png)

---

## Part 4: Resource Naming Convention

To manage resources easily, use a naming convention:
...
**Benefit**: Easy to find & manage resources in the console.

## Troubleshooting

### Cannot Create IAM User

- **Check**: Are you logged in with the root account or another IAM user?
- **Solution**: Log in again with the root account to create the IAM user.

### Permission Denied Errors

- **Verify**: IAM policies attached to the user.
- **Check**: Do the policies include the service you are using?
- **Contact**: AWS support if you need elevated permissions.

---

## Notes

- From now on, all actions will use the IAM user, not the root account.
- Each service will have a specific IAM role (created in the next modules).
- The Free Tier provides sufficient resources for learning.
...
2. An IAM user "workshop-admin" with admin privileges
3. Access to all necessary AWS services
